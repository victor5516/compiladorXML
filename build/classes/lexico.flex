...
espacio		= [ \t]+
%%
"<si><condicion"            {	if(debug) System.out.println("token IF");
			return sf.newSymbol("IF",sym.IF);
			}
"<entonces>"          { if(debug) System.out.println("token THEN");
			return sf.newSymbol("THEN",sym.THEN);
			}
"<sino>"          {	if(debug) System.out.println("token ELSE");
			return sf.newSymbol("ELSE",sym.ELSE);
			}
"<entonces/>"           {	if(debug) System.out.println("token END");
			return sf.newSymbol("END",sym.END);
			}
"<repita>"        {	if(debug) System.out.println("token REPEAT");
			return sf.newSymbol("REPEAT",sym.REPEAT);
			}
"<repita/><condicion"         {	if(debug) System.out.println("token UNTIL");
			return sf.newSymbol("UNTIL",sym.UNTIL);
			}
"<leer"          {	if(debug) System.out.println("token READ");
			return sf.newSymbol("READ",sym.READ);
			}
"<operacion"    {if(debug) System.out.println("token OP");
			return sf.newSymbol("READ",sym.OP);
		}

"/><si/>"    {if(debug) System.out.println("token FINSI");
			return sf.newSymbol("READ",sym.FINSI);
		}

"<sino/>"   {if(debug) System.out.println("token FINELSE");
			return sf.newSymbol("READ",sym.FINELSE);}

"<escribir"         {	if(debug) System.out.println("token WRITE");
